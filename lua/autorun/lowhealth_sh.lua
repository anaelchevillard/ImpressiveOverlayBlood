...
CreateConVar( "etb_threshold", 35, {FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY}, "At this point, sounds and screen will getting stronger" )
...
CreateConVar( "etb_muffle_effect", 35, {FCVAR_SERVER_CAN_EXECUTE, FCVAR_NOTIFY}, "Health value that the sounds will become muffled" )
		...
		if intensity > 0 then
			...
			if ply:Alive() then
				...
				if etb_heartbeat_sound:GetInt() == 1 then
					if CT > hpwait then
						ply:EmitSound("lowhp/hbeat.wav", 70 * intensity, 100 + 20 * intensity)
						hpwait = CT + 0.5
					end
				end
